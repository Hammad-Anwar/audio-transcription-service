...
public class WhisperTranscriptionService implements TranscriptionService {
    ...
    private static TranscriptionResult normalize(WhisperResponse response) {
        if (response == null
        ...
        for (int index = 0; index < response.segments().size(); index++) {
            ...
            normalized.add(new TranscriptionSegmentResult(
                    index,
                    startMs,
                    endMs,
                    segment.text().trim()
        ...
        return new TranscriptionResult(language, durationMs, normalized);
    }
